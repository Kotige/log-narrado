  ...
  return (
    <>
      <header className="sticky top-0 z-50 w-full bg-bg">
        ...
        {/* Nav — mobile */}
        <nav
          className={`absolute lef-0 right-0 top-full flex flex-col overflow-hidden text-right  bg-bg px-8 shadow-sm transition-[max-height,width,opacity] duration-500 ease-in-out md:hidden ${
            menuOpen
              ? "max-h-96 w-100 pb-5 pt-2 opacity-100"
              : "max-h-0 w-0 pb-0 pt-0 opacity-0"
          }`}
        >
          {NAV_LINKS.map((link) => {
            const isActive = link.href === activeHref;
            return (
              <a
                ...
                href={link.href}
                className={`
                  px-1.5 py-3 text-[14.5px] no-underline
                  ${isActive ? "text-moss-dark font-extrabold" : "text-ink font-medium"}
                `}
              >
                {link.label}
              </a>
      ...
      {menuOpen && (
        <div
          ...
          aria-hidden="true"
          className="fixed inset-0 z-40 bg-ink/10 backdrop-blur-sm transition-opacity duration-300 md:hidden"
        />
      )}
    </>
  );
